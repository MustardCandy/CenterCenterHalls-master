...
class Utils {
  ...
  */
  ...
  /*whitelist(item, whitelist, call="whitelist");
  @param item: {mixed} an item to check for the presence of in the whitelist
  @param whitelist: {array} the master list to check against
  @pram call: {string} the local of the error.
  @return: {array} returns item if no error
  */
  static whitelist(item, whitelist, call="whitelist"){
    whitelist = this.typeCheck(whitelist, "array", call);
    if(whitelist.indexOf(item) === -1){ throw new Error( "Unexpected item " + item + " passed to method " + call + "."); }
    return item;
  ...
  */
  static coordinateMorphs(type="all"){
    var baseCases = [{x: 1, y: 0}, {x: 0, y: 1}, {x:-1, y: 0}, {x: 0, y: -1}];
    var diagCases = [{x: 1, y: 1}, {x: 1, y: -1}, {x: -1, y: 1}, {x: -1, y: -1}];
