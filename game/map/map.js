...
class Map{
  ...
  */
  ...
  /*getOpen(type)
  @param type:{string} the type of Open thing to be returned, hall, room or all
  @return: {array} an array of objects with x and y keys
  */
  getOpen(type){
    type = Utils.whitelist(type, ["hall", "room", "all"], "Map.getOpen");
    var openCells = [];
    for (var i = 0; i < this.map.length; i++) {
      for (var j = 0; j < this.map[i].length; j++) {
        var rType = this.map[i][j].type
        if((type === "room" || type === "all") && rType === "room"){ openCells.push({y: i, x: j}); }
        if((type === "hall" || type === "all") && rType === "hall"){ openCells.push({y: i, x: j}); }
      }
    }
  ...
  @param coordinates: {object} an x and y keyed coordinate
  @param diagonal: {boolean} should check diagonal or no
  @return an array of open cells
  */
  open(coordinates, diagonal=true){
    coordinates = Utils.typeCheck(coordinates, "obj", "Map.open");
    diagonal = Utils.typeCheck(diagonal, "bool", "Map.open");
    var testCases = [];
    if(diagonal){ testCases = Utils.coordinateMorphs("all"); }
    else { testCases = Utils.coordinateMorphs("base"); }
    var returnArray = []
    for (var i = 0; i < testCases.length; i++) {
      var test = testCases[i];
      var testCell = {x: coordinates.x + test.x, y: coordinates.y + test.y};
